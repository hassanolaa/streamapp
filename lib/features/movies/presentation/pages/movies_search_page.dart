...
import 'package:streamapp/features/movies/presentation/widgets/movie_grid_widget.dart';

class MoviesSearchPage extends StatelessWidget {
  final String? initialQuery;
  const MoviesSearchPage({super.key, this.initialQuery});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => MoviesCubit(repository: sl()),
      child: _MoviesSearchContent(initialQuery: initialQuery),
    );
  }
}

class _MoviesSearchContent extends StatefulWidget {
  final String? initialQuery;
  const _MoviesSearchContent({this.initialQuery});

  @override
  State<_MoviesSearchContent> createState() => _MoviesSearchContentState();
}

class _MoviesSearchContentState extends State<_MoviesSearchContent> {
  // ── Focus nodes ──
  final FocusNode _backButtonFocusNode  = FocusNode();
  final FocusNode _searchFocusNode      = FocusNode(); // wraps text field
  final FocusNode _textFieldFocusNode   = FocusNode(); // actual TextField
  ...
  GenreModel?     _selectedGenre   = null;
  ...
  Future<void> _loadGenres() async {
    try {
      final genres = await sl<MoviesRepository>().getGenres();
  ...
  void _onScroll() {
    ...
    if (state is MoviesSearchSuccess && state.hasMore) {
      _isFilterMode
  ...
  void _applyFilters() {
    ...
    context.read<MoviesCubit>().discoverMovies(
  ...
  // ── Keyboard handler (top-bar wrapper Focus) ──
  KeyEventResult _handleTopBar(KeyEvent event) {
    if (event is! KeyDownEvent) return KeyEventResult.ignored;

    // Grid is focused → only catch Escape to come back
    ...
    // Text field is focused → only intercept navigation keys
    if (_topBarIndex == 1) {
      ...
      if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
        final s = context.read<MoviesCubit>().state;
        if (s is MoviesSearchSuccess || s is MoviesLoadingMore) {
      ...
      return KeyEventResult.ignored; // let TextField handle typing / Enter
    }
  ...
  Widget build(BuildContext context) {
    return Shortcuts(
      shortcuts: {LogicalKeySet(LogicalKeyboardKey.escape): const _BackIntent()},
      child: Actions(
        actions: {
  ...
  Widget _buildResults() {
    return BlocBuilder<MoviesCubit, MoviesState>(
      builder: (context, state) {
        if (state is MoviesInitial) return _buildEmptyPrompt();
        if (state is MoviesLoading)  return const Center(child: CircularProgressIndicator());
        if (state is MoviesError)    return _buildError(state.message);

        if (state is MoviesSearchSuccess || state is MoviesLoadingMore) {
          final movies = state is MoviesSearchSuccess
              ? state.movies
              : (state as MoviesLoadingMore).currentMovies;

          if (movies.isEmpty) return _buildNoResults();

          final total = state is MoviesSearchSuccess ? state.totalResults : movies.length;
          ...
          return Column(children: [
            ...
            Expanded(
              child: MovieGridWidget(
                ...
                onMovieTap: (movie) => Navigator.push(context,
                    MaterialPageRoute(
            ...
            children: [
              _dropdown<String>(context,
                  ...
                  items: _sortOptions.entries
                      ...
                      .toList(),
                  onChanged: (v) => setState(() => _selectedSort = v ?? 'popularity.desc'),
                  width: 210),

              _dropdown<GenreModel?>(context,
                  label: 'Genre',
    ...
    return Wrap(spacing: 8, children: [
      if (_selectedGenre != null)
        Chip(
          ...
          onDeleted: () { setState(() => _selectedGenre = null); _applyFilters(); },
      ...
      if (_selectedYear != null)
        Chip(
          ...
          deleteIcon: const Icon(Icons.close_rounded, size: 16),
          onDeleted: () { setState(() => _selectedYear = null); _applyFilters(); },
          backgroundColor: Theme.of(context).colorScheme.primaryContainer,
          side: BorderSide.none,
        ),
      if (_selectedMinRating != null)
        Chip(
          ...
          deleteIcon: const Icon(Icons.close_rounded, size: 16),
          onDeleted: () { setState(() => _selectedMinRating = null); _applyFilters(); },
          backgroundColor: Theme.of(context).colorScheme.primaryContainer,
          side: BorderSide.none,
        ),
    ]);
    ...
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.movie_filter_rounded,
            size: 80, color: Theme.of(context).textTheme.bodyMedium!.color),
        const SizedBox(height: 16),
        Text('Search for movies or use filters',
    ...
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.movie_outlined,
            size: 80, color: Theme.of(context).textTheme.bodyMedium!.color),
        const SizedBox(height: 16),
        Text('No movies found', style: Theme.of(context).textTheme.titleLarge),
      ]),
    );
  }
